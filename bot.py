...
class AIBot(commands.Bot):
    ...
    async def send_startup_notification(self, synced_commands: int = 0):
        """Send startup notification to stats report channel"""
        if not self.stats_report_channel_id:
            logger.debug("Startup notification skipped: no stats report channel configured")
            return

        try:
            channel = self.get_channel(self.stats_report_channel_id)
            if not channel or not isinstance(channel, discord.TextChannel):
                logger.warning(f"Startup notification failed: channel {self.stats_report_channel_id} not found or not a text channel")
                return
            ...
            # Redis status
            redis_status = "Conectado" if self.redis_client else "Desconectado"

            # Get token stats if available
            total_tokens_str = "N/A"
            total_cost_str = "N/A"
            if self.stats_cog and self.redis_client and self.guilds:
                try:
                    # Get stats from the first guild (or aggregate if needed)
                    ...
                    # Use average cost estimation
                    avg_cost_per_million = 0.375  # Rough average
                    ...
                    logger.error(f"Error fetching token stats for startup: {e}")

            # Create embed
            embed = discord.Embed(
                title="🟢 Bot Iniciado",
                description="¡El bot se ha iniciado correctamente y está listo para servir!",
            ...
            server_info = (
                ...
                f"Hora de Inicio: <t:{int(self.start_time)}:F>"
            )
            database_info = (
            ...
            embed.add_field(
                ...
                value=(
                    "Servidor de Salud: Ejecutándose\n"
                    "Scheduler: Activo\n"
                    f"Comandos: {synced_commands} sincronizados"
                ),
                inline=False
            )

            embed.set_footer(text=f"{self.user.name} • Startup Notification")  # type: ignore

            await channel.send(embed=embed)
            logger.info(f"Startup notification sent to channel {self.stats_report_channel_id}")

        except Exception as e:
            logger.error(f"Failed to send startup notification: {e}", exc_info=True)

    async def send_shutdown_notification(self):
        """Send shutdown notification to stats report channel"""
        if not self.stats_report_channel_id:
            logger.debug("Shutdown notification skipped: no stats report channel configured")
            return

        try:
            channel = self.get_channel(self.stats_report_channel_id)
            if not channel or not isinstance(channel, discord.TextChannel):
                logger.warning(f"Shutdown notification failed: channel {self.stats_report_channel_id} not found")
                return
            ...
            if self.stats_cog and self.redis_client and self.guilds:
                try:
                    ...
                    daily_key = f"tokens:guild:{guild_id}:daily"
                    daily_tokens = await asyncio.to_thread(self.redis_client.get, daily_key)
                    daily_tokens = int(daily_tokens) if daily_tokens else 0

                    # Get today's cached tokens
                    daily_cached_key = f"tokens:guild:{guild_id}:daily_cached"
                    daily_cached = await asyncio.to_thread(self.redis_client.get, daily_cached_key)
                    daily_cached = int(daily_cached) if daily_cached else 0

                    # Calculate cost
                    billed_tokens = daily_tokens - daily_cached
                    avg_cost_per_million = 0.375
                    session_cost = (billed_tokens / 1_000_000) * avg_cost_per_million

                    session_tokens_str = f"{daily_tokens:,} ({daily_cached:,} cached)"
                    session_cost_str = f"${session_cost:.4f}"
                except Exception as e:
                    logger.error(f"Error fetching session stats for shutdown: {e}")

            # Determine shutdown type
            shutdown_type = "Señal de apagado" if self.shutdown_signal else "Desconocido"

            # Create embed
            embed = discord.Embed(
                title="🔴 Apagado del Bot",
                description=f"El bot {self.user.name} se está apagando correctamente...",  # type: ignore
            ...
            embed.add_field(
                ...
                value=f"Tiempo Activo: {uptime_str}",
                inline=False
            )

            # Token consumption for this session
            embed.add_field(
                name="🎯 Consumo de Sesión",
                value=(
                    f"Tokens: {session_tokens_str}\n"
                    f"Costo: {session_cost_str}"
                ),
                inline=False
            )
            ...
            embed.add_field(
                ...
                value=(
                    f"Tipo: {shutdown_type}\n"
                    "Recursos: Limpiando\n"
                    "Health: Puerto cerrando"
                ),
                inline=False
            )

            embed.set_footer(text=f"{self.user.name} • Shutdown Notification")  # type: ignore

            await channel.send(embed=embed)
            logger.info(f"Shutdown notification sent to channel {self.stats_report_channel_id}")

        except Exception as e:
            logger.error(f"Failed to send shutdown notification: {e}", exc_info=True)
